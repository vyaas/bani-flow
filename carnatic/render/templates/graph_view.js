...
const cy = cytoscape({
  ...
  style: [
    ...
    // ADR-172: the four per-shape `border-width: 3px` rules are gone. They
    // existed only so non-circular geometry read clearly; with uniform circles
    // the 2px base border applies everywhere.
    {
...
// ── ERA_COLOURS mirror (for chip injection) ───────────────────────────────────
// ERA_COLOURS now sourced from THEME.era (ADR-028: single source of truth)
const ERA_COLOURS = THEME.era;

// ── Instrument glyphs (ADR-172) ───────────────────────────────────────────────
// The old INSTRUMENT_SHAPES table and makeShapeSVG() geometry factory are gone.
// Instruments are drawn silhouettes now, defined once in
// carnatic/render/instruments.py, emitted into the inline SVG sprite at build
// time, and referenced here by id. INSTRUMENTS (label/order/tradition/filter)
// is generated by the same module and injected before this file.
//
// NB: <use href="#id"> resolves inside inline SVG — but CSS mask: url("#id")
// does not (see the comment at the .lecdem-chip rule). Keep DOM glyphs on <use>.

function instrumentSymbolId(instrKey) {
  const key = (instrKey && INSTRUMENTS[instrKey]) ? instrKey : INSTRUMENT_FALLBACK;
  return INSTRUMENT_SYMBOL_PREFIX + key;
}

// Registry keys ordered for display. Always excludes the presentation-only
// fallback (`unknown` is never a value a user can pick or filter on).
//   opts.filterable — only instruments flagged for the artist filter dropdown
//   opts.tradition  — only instruments of that tradition
function instrumentKeys(opts) {
  const o = opts || {};
  return Object.keys(INSTRUMENTS)
    .filter(k => k !== INSTRUMENT_FALLBACK)
    .filter(k => !o.filterable || INSTRUMENTS[k].filter)
    .filter(k => !o.tradition  || INSTRUMENTS[k].tradition === o.tradition)
    .sort((a, b) => INSTRUMENTS[a].order - INSTRUMENTS[b].order);
}

function instrumentLabel(instrKey) {
  const entry = INSTRUMENTS[instrKey];
...
function buildFilterDropdowns() {
  ...
  // ADR-172: order and labels come from the generated INSTRUMENTS registry, so
  // adding an instrument to instruments.py surfaces it here automatically.
  const instrOrder = instrumentKeys({ filterable: true });
  ...
  instrOrder.forEach(instr => {
    ...
    iconWrap.appendChild(makeInstrIconSVG(instr, 12));

    const labelSpan = document.createElement('span');
    labelSpan.textContent = instrumentLabel(instr);
...
(function () {
  ...
  cy.on('tap', 'edge[kind = "transitive"]', function (evt) {
    ...
    transitIds.forEach(function (tid, i) {
      ...
      // ADR-172: transit chips carry the instrument glyph like every other
