...
function orientRagaWheel(type, id) {
  ...
  function waitAndPan() {
    ...
    // Resolve the target point in vp-space (unscaled wheel coordinates).
    // For a composition click: zoom to the comp node itself (further out, tighter zoom).
    // For a raga/mela click: zoom to the mela node as before.
    let targetX, targetY, TARGET_SCALE;

    if (type === 'comp') {
      // The comp node circle has cx/cy set in vp-space — read them directly.
      const compCircle = document.querySelector(
        `#wheel-viewport .comp-node[data-id="${id}"] circle`
      );
      if (compCircle) {
        targetX = parseFloat(compCircle.getAttribute('cx'));
        targetY = parseFloat(compCircle.getAttribute('cy'));
      } else {
        // Comp node not yet in DOM (e.g. mela-direct with no janya) — fall back to mela
        const R_MELA = minDim * 0.38;
      ...
      TARGET_SCALE = 2.8;   // compositions are at R_COMP ≈ 0.72·minDim — zoom in more
    } else {
      const R_MELA = minDim * 0.38;
      const melaAngle = (melaRaga.melakarta - 1) * 5;
      const rad = (melaAngle - 90) * Math.PI / 180;
