...
// ADR-096: solveRingLayout removed — fan geometry replaced by arc-slice subdivision.
...
function _startTapHoldTimer(e) {
  ...
  _tapHoldTimer = setTimeout(() => {
    ...
    if (!el) return;
    // Determine node type from containing group class (ADR-096: mela-arc replaces mela-node)
    const g = el.closest('.mela-arc, .janya-arc-group, .comp-arc-group');
    if (!g) return;
    let nodeType, nodeId;
    if (g.classList.contains('mela-arc')) {
      nodeType = 'mela';
      const melaId = g.getAttribute('data-id');
      nodeId = melaId || null;
    } else if (g.classList.contains('janya-arc-group')) {
      nodeType = null;  // janya detail is in WDP panel — no inspector on arc
    } else if (g.classList.contains('comp-arc-group')) {
      nodeType = null;  // comp detail via triggerBaniSearch — no inspector on arc
    }
...
window.drawRagaWheel = function() {
  ...
  // ADR-096: sunburst radii — all three levels fit within 0.70 × minDim
  const R_INNER       = minDim * 0.08;   // cakra hub (unchanged)
  const R_CAKRA       = minDim * 0.20;   // cakra sector outer — wider for label headroom
  const R_MELA_OUTER  = minDim * 0.38;   // mela arc outer rim
  const R_JANYA_OUTER = minDim * 0.56;   // janya sub-arc outer rim
  const R_COMP_OUTER  = minDim * 0.70;   // comp sub-arc outer rim

  RagaWheel._geometry.cx = cx;
  RagaWheel._geometry.cy = cy;
  RagaWheel._geometry.rOuter = R_MELA_OUTER;  // rim-drag boundary = mela arc edge
  ...
  const melaLabelFontSize = Math.max(7, minDim * 0.013); // radial spoke font (ADR-096)
  ...
  // ADR-096: 72 mela arc-slices (sunburst formalism) ──────────────────────────
  // Each mela occupies a fixed 5° wedge spanning R_CAKRA → R_MELA_OUTER.
  // Labels are radial spoke text outside R_MELA_OUTER (no chip pills).
  ...
  for (let n = 1; n <= 72; n++) {
    ...
    const origOpacity = isLive ? 0.85 : (raga ? 0.25 : 0.4);

    const g = svgEl('g', { class: 'mela-arc', 'data-mela': n, 'data-id': raga ? raga.id : '' });

    // Arc-slice: spans R_CAKRA → R_MELA_OUTER within the 5° wedge
    const arc = svgEl('path', {
      d: sectorPath(cx, cy, R_CAKRA, R_MELA_OUTER, startDeg, endDeg),
      fill: raga ? color : THEME.bgPanel,
      stroke: THEME.labelOutline,
      'stroke-width': 0.5,
      opacity: origOpacity,
    ...
    // Fat transparent hit-target arc for touch accuracy (ADR-035 §7)
    g.appendChild(svgEl('path', {
      d: sectorPath(cx, cy, R_CAKRA - 4, R_MELA_OUTER + 4, startDeg, endDeg),
    ...
    if (isLive) {
      ...
      g.addEventListener('mouseenter', () => {
        ...
        const ap = polar(cx, cy, (R_CAKRA + R_MELA_OUTER) / 2, midDeg);
        showWheelTooltip(svg, ap.x, ap.y, lines);
      });
      ...
      g.addEventListener('click', (e) => {
        ...
        if (_expandedMela === raga.id) {
          ...
          arc.setAttribute('stroke', THEME.accentSelect);
          arc.setAttribute('stroke-width', '1.5');
          _expandedMela = raga.id;
          // Dim all other mela arcs
          vp.querySelectorAll('.mela-arc path[data-mela]').forEach(p => {
            const melaG = p.closest('.mela-arc');
            const nodeId = melaG ? melaG.getAttribute('data-id') : '';
            if (nodeId !== raga.id) p.setAttribute('opacity', '0.12');
          });
          // Dim all other spoke labels
          if (_labelLayer) {
            _labelLayer.querySelectorAll('.mela-label').forEach(lbl => {
              if (lbl.getAttribute('data-id') !== raga.id)
                lbl.setAttribute('opacity', '0.10');
            });
          }
          // Draw janya sub-arcs in SVG (ADR-096 sunburst expansion)
          _expandMelaSunburst(vp, svg, raga, startDeg, endDeg, cx, cy,
            R_MELA_OUTER, R_JANYA_OUTER, R_COMP_OUTER, color, minDim);
          window._wheelSyncInProgress = true;
          if (!window._wheelPreviewNoPanel && typeof applyBaniFilter === 'function') {
            applyBaniFilter('raga', raga.id);
          }
          window._wheelSyncInProgress = false;
          // Zoom to arc centroid
          const ap = polar(cx, cy, (R_CAKRA + R_MELA_OUTER) / 2, midDeg);
          _animateToTarget(ap.x, ap.y, 2.5);
        }
      });
    ...
    // Radial spoke label outside R_MELA_OUTER
    const spokePad = Math.max(4, minDim * 0.008);
    const lp = polar(cx, cy, R_MELA_OUTER + spokePad, midDeg);
    const normMid = ((midDeg % 360) + 360) % 360;
    let spokRotDeg, textAnchor;
    if (normMid < 1 || normMid > 359)    { spokRotDeg = 0;           textAnchor = 'middle'; }
    else if (Math.abs(normMid - 180) < 1){ spokRotDeg = 0;           textAnchor = 'middle'; }
    else if (normMid < 180)              { spokRotDeg = midDeg - 90; textAnchor = 'start';  }
    else                                 { spokRotDeg = midDeg + 90; textAnchor = 'end';    }
    const spokeLabel = raga ? abbrev(raga.name, 5) : String(n);
    const lblOpacity = isLive ? 0.85 : (raga ? 0.30 : 0.40);
    const lbl = svgEl('text', {
      x: lp.x, y: lp.y,
      'text-anchor': textAnchor, 'dominant-baseline': 'middle',
      fill: isLive ? THEME.fg : (THEME.fgMuted || THEME.borderStrong),
      'font-size': melaLabelFontSize + 'px',
      opacity: lblOpacity,
      'data-orig-opacity': lblOpacity,
      class: 'mela-label', 'data-id': raga ? raga.id : '',
      transform: `rotate(${spokRotDeg}, ${lp.x}, ${lp.y})`,
    });
    lbl.textContent = spokeLabel;
...
function _collapseAll(vp, melaByNum) {
  _closeWheelDetailPanel();
  // Remove janya and comp arc sub-groups (ADR-096 sunburst)
  vp.querySelectorAll('.janya-arc-group, .comp-arc-group').forEach(g => g.remove());
  // Restore mela arc opacity and stroke
  vp.querySelectorAll('.mela-arc path[data-mela]').forEach(p => {
    p.setAttribute('stroke', THEME.labelOutline);
    p.setAttribute('stroke-width', '0.5');
    const orig = p.getAttribute('data-orig-opacity');
    if (orig) p.setAttribute('opacity', orig);
  });
  // Restore mela spoke label opacity
  if (_labelLayer) {
    _labelLayer.querySelectorAll('.mela-label').forEach(lbl => {
...
// ADR-096: Draw janya sub-arcs for the selected mela within its 5° wedge.
// janyas are subdivided with equal angular slices: sliceDeg = (endDeg - startDeg) / n_janyas.
function _expandMelaSunburst(vp, svg, raga, melaStartDeg, melaEndDeg, cx, cy,
    R_MELA_OUTER, R_JANYA_OUTER, R_COMP_OUTER, melaColor, minDim) {
  if (!_wdpData) return;
  const { janyasByMela } = _wdpData;
  const janyas = (janyasByMela[raga.id] || []).slice()
    .sort((a, b) => (a.name || '').localeCompare(b.name || ''));
  if (janyas.length === 0) return;

  const g = svgEl('g', { class: 'janya-arc-group', 'data-parent': raga.id });
  const wedgeDeg = melaEndDeg - melaStartDeg;  // always 5°
  const sliceDeg = wedgeDeg / janyas.length;

  janyas.forEach((janya, i) => {
    const jStartDeg = melaStartDeg + i * sliceDeg;
    const jEndDeg   = jStartDeg + sliceDeg;
    const jMidDeg   = jStartDeg + sliceDeg / 2;

    const jArc = svgEl('path', {
      d: sectorPath(cx, cy, R_MELA_OUTER, R_JANYA_OUTER, jStartDeg, jEndDeg),
      fill: melaColor,
      stroke: THEME.labelOutline,
      'stroke-width': 0.5,
      opacity: 0.55,
    ...
    jArc.addEventListener('mouseenter', () => {
      const comps = (_wdpData.compsByRaga[janya.id] || []).length;
      const lines = [janya.name, 'Janya of ' + raga.name];
      if (comps) lines.push(comps + ' composition' + (comps > 1 ? 's' : ''));
      const tp = polar(cx, cy, (R_MELA_OUTER + R_JANYA_OUTER) / 2, jMidDeg);
      showWheelTooltip(svg, tp.x, tp.y, lines);
    });
    jArc.addEventListener('mouseleave', hideWheelTooltip);
    jArc.addEventListener('click', (e) => {
      e.stopPropagation();
      // Dim siblings, highlight this arc
      g.querySelectorAll('path').forEach(p => p.setAttribute('opacity', '0.20'));
      jArc.setAttribute('opacity', '0.90');
      jArc.setAttribute('stroke', THEME.accentSelect);
      jArc.setAttribute('stroke-width', '1');
      // Expand comp sub-arcs
      vp.querySelectorAll('.comp-arc-group').forEach(el => el.remove());
      _expandJanyaSunburst(vp, svg, janya, jStartDeg, jEndDeg, cx, cy,
        R_JANYA_OUTER, R_COMP_OUTER, melaColor, minDim);
      // Mirror selection in WDP panel (text labels)
      const chipEl = document.querySelector(
        `#wdp-janya-list .wdp-chip.wdp-raga[data-id="${CSS.escape(janya.id)}"]`
      );
      _wdpSelectJanya(janya, chipEl, false, null);
      _expandedJanya = janya.id;
      // Zoom toward the janya arc centroid
      const jp = polar(cx, cy, (R_MELA_OUTER + R_JANYA_OUTER) / 2, jMidDeg);
      _animateToTarget(jp.x, jp.y, 2.2);
    });
...
// ADR-096: Draw comp sub-arcs for the selected janya within its angular slice.
function _expandJanyaSunburst(vp, svg, janya, jStartDeg, jEndDeg, cx, cy,
    R_JANYA_OUTER, R_COMP_OUTER, parentColor, minDim) {
  if (!_wdpData) return;
  const items = _wdpData.compsByRaga[janya.id] || [];
  if (items.length === 0) return;

  const g = svgEl('g', { class: 'comp-arc-group', 'data-parent': janya.id });
  const wedgeDeg = jEndDeg - jStartDeg;
  const sliceDeg = wedgeDeg / items.length;

  items.forEach((item, i) => {
    const cStartDeg = jStartDeg + i * sliceDeg;
    const cEndDeg   = cStartDeg + sliceDeg;
    const cMidDeg   = cStartDeg + sliceDeg / 2;

    const cArc = svgEl('path', {
      d: sectorPath(cx, cy, R_JANYA_OUTER, R_COMP_OUTER, cStartDeg, cEndDeg),
      fill: THEME.accent,
      stroke: THEME.labelOutline,
      'stroke-width': 0.5,
      opacity: 0.65,
    ...
    cArc.addEventListener('mouseenter', () => {
      const header = (item._isPerf && item._fullLabel) ? item._fullLabel : (item.title || '');
      const lines = [header];
      if (item.concert) lines.push(item.concert);
      if (item.date || item.year) lines.push(item.date || String(item.year));
      const tp = polar(cx, cy, (R_JANYA_OUTER + R_COMP_OUTER) / 2, cMidDeg);
      showWheelTooltip(svg, tp.x, tp.y, lines);
    });
    cArc.addEventListener('mouseleave', hideWheelTooltip);
    cArc.addEventListener('click', (e) => {
      e.stopPropagation();
      // Highlight selected arc
      g.querySelectorAll('path').forEach(p => p.setAttribute('opacity', '0.25'));
      cArc.setAttribute('opacity', '1');
      cArc.setAttribute('stroke', THEME.accentSelect);
      cArc.setAttribute('stroke-width', '1');
      _expandedComp = item.id;
      // Trigger bani-flow search
      window._wheelSyncInProgress    = true;
      window._wheelOriginatedTrigger = true;
      if (!item._isPerf) {
        if (typeof triggerBaniSearch === 'function') triggerBaniSearch('comp', item.id);
        ...
        if (typeof triggerBaniSearch === 'function') triggerBaniSearch('yt', item._ytVid + '::' + (item.raga_id || janya.id));
      } else {
        if (typeof triggerBaniSearch === 'function') triggerBaniSearch('raga', item.raga_id || janya.id);
      }
      window._wheelSyncInProgress    = false;
      window._wheelOriginatedTrigger = false;
    });
...
function _animateToTarget(targetX, targetY, targetScale) {
  ...
  // ADR-096: opens the detail panel for the given mela and pre-selects the janya/comp.
  window._triggerMelaExpand = function(melaNum, targetRagaId, targetCompId) {
    ...
    // Highlight the mela arc on the wheel ring.
    ...
    const melaG = document.querySelector(`#wheel-viewport .mela-arc[data-mela="${melaNum}"]`);
...
function orientRagaWheel(type, id) {
  ...
  function waitAndPan() {
    ...
    const H = svg.clientHeight || svg.parentElement.clientHeight || 600;
    const minDim = Math.min(W, H);
    // ADR-096: target is the arc centroid at midpoint of (R_CAKRA=0.20 + R_MELA_OUTER=0.38)/2
    const R_ARC_CENTROID = minDim * 0.29;
    const melaAngle = (melaRaga.melakarta - 1) * 5 + 2.5;  // arc midpoint angle (degrees)
    const rad = (melaAngle - 90) * Math.PI / 180;
    const TARGET_SCALE = 2.5;
    const targetX = W / 2 + R_ARC_CENTROID * Math.cos(rad);
    const targetY = H / 2 + R_ARC_CENTROID * Math.sin(rad);
