...
**Implementation note (2026-06-12, Coder — final):** the below-video footer is **fully retired**, as §2 intends. The first pass kept `.mp-footer` for the lecdem subject index, but a follow-up (per user direction) moved lecdem subjects *into the rail too*: `_buildLecdemSubjectFooter` was refactored into `_buildSubjectChips(subjects)` (returns chips, not a footer), and `buildPlayerRail` accepts `meta.subjects`, so a lecture's raga/composition/musician cross-links populate the header rail and spill into the same `▾ N` overflow as everything else (§5). All three lecdem open-paths pass `subjects` in meta; subjects persist across segment updates. No code creates `.mp-footer` any longer — only a few inert mentions remain in collapsed-state CSS selector lists (flagged for cleanup). The chip order in the rail is **performer · raga · composition · composer · [subjects] · tala**, with **tala demoted to the very end** (unbundled from the raga chip) and shown only when present.
